...
/**
 * Creates a microsecond conversion wrapper.
 * @param   {number}    value the value in microseconds to be wrapped.
 * @returns {Converter}       a converter object.
 */
export default function microseconds(value) {
  return units.reduce(
    (definition, current) =>
      Object.defineProperty(definition, `to${current.substring(0, 1).toUpperCase() + current.slice(1)}`, {
        enumerable: true,
        configurable: false,
        value: () => value * ratios['microseconds'].conversion[ratios[current].position],
      }),
    {},
  );
}

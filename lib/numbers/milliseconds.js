...
/**
 * Creates a millisecond conversion wrapper.
 * @param   {number}    value the value in milliseconds to be wrapped.
 * @returns {Converter}       a converter object.
 */
export default function milliseconds(value) {
  return units.reduce(
    (definition, current) =>
      Object.defineProperty(definition, `to${current.substring(0, 1).toUpperCase() + current.slice(1)}`, {
        enumerable: true,
        configurable: false,
        value: () => value * ratios['milliseconds'].conversion[ratios[current].position],
      }),
    {},
  );
}
